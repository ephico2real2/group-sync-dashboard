...
### R5-1 corrected after research — the recovery shape retracted by the business owner

The R5-1 fix as first specified (and shipped in `9a40691`: read every `Location` value and every
`#`-segment, adopt the first fragment carrying a token) was **error recovery from a malformed
construct**, and the business owner retracted it after checking the standard rather than reasoning
from first principles. RFC 9110 §5.5 (https://www.rfc-editor.org/rfc/rfc9110.html) classifies
`Location` as a **singleton field**: a response carrying more than one is malformed, not a response
with two candidate tokens, and the RFC says of clients like this one that "a systems control client
might consider any form of error recovery to be dangerous". Recovery is also attacker-steerable — a
hostile target sending `…#access_token=ATTACKER_CHOSEN` before the real one decides which token
becomes the cluster credential under a "first one carrying a token" rule.

| decision | what landed instead — less code |
|---|---|
| more than one `Location` ⇒ malformed ⇒ a typed terminal stop | `_token_from` counts the values with `get_list` (count, never choose) and raises `LoginError(UNREACHABLE, …malformed (RFC 9110 §5.5)…, retryable=False)`; no session is ever built from such a response |
| best-effort revoke anyway | the guard's `_minted_by` returns what each value carried; each is revoked once with `best_effort=true` on the line; `revoked` is True only if the target said gone for all, False if any failed, **None when nothing could be named** — and the stop message says that litter is #286's, so it is logged rather than dropped |
| exactly one `Location` ⇒ the simple path | `_fragment_of(value)`: first `#`, `parse_qs` — unchanged; a value a proxy joined itself is one malformed value, not two candidates, and is not recovered |
| `_fragments_of`, `_token_in`, the candidate selection | **deleted** |

Evidence that refusing breaks a real OpenShift behaviour: none found — the flow measured on the
reference cluster (§2 of the spec) sends exactly one `Location`. The implicit-flow note (RFC 9700 /
OAuth 2.1 deprecate `response_type=token`; its threats — browser history, `Referer` — do not apply
to a non-browser that never follows the redirect) is recorded in the spec's §2.2 and the module
docstring, so a future reader does not "fix" a flow OpenShift's CLI login path defines.
