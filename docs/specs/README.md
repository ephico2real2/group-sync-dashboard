...
| C1 | [`SPEC_C1_table_export.md`](SPEC_C1_table_export.md) — CSV and JSON export of the table on screen | C — product | R3 | app 0.14.0, chart 0.15.0 | [#61](https://github.com/ephico2real2/group-sync-dashboard/issues/61) | released |
| C2 | [`SPEC_C2_users_tab_providers_identities.md`](SPEC_C2_users_tab_providers_identities.md) — Users tab: provider allow-list and first login from Identity objects | C — product | R3 | app 0.15.0, chart 0.16.0 | [#62](https://github.com/ephico2real2/group-sync-dashboard/issues/62) | released |
| A2 | [`SPEC_A2_supply_chain.md`](SPEC_A2_supply_chain.md) — SBOM, keyless signing, build and chart provenance | A — supply chain | R4 | no app or chart version change (workflows and the build script only) | [#63](https://github.com/ephico2real2/group-sync-dashboard/issues/63) | released |
| B1 | [`SPEC_B1_offsite_backup.md`](SPEC_B1_offsite_backup.md) — off-volume backup CronJob and restore runbook | B — operations | R4 | chart 0.17.0 (chart only) | [#64](https://github.com/ephico2real2/group-sync-dashboard/issues/64) | released |
| C4 | [`SPEC_C4_idle_timeout.md`](SPEC_C4_idle_timeout.md) — idle timeout with countdown | C — product | R5 | app 0.16.0, chart 0.18.0 | [#65](https://github.com/ephico2real2/group-sync-dashboard/issues/65) | released |
| D1 | [`SPEC_D1_audit_log_login_capture.md`](SPEC_D1_audit_log_login_capture.md) — login capture from the oauth-server audit log | D — architecture | R5 | app 0.17.0, chart 0.19.0 | [#66](https://github.com/ephico2real2/group-sync-dashboard/issues/66) | released |
| C3 | [`SPEC_C3_reporting_microservice.md`](SPEC_C3_reporting_microservice.md) — reporting as a microservice: the report service, its catalogue, the dashboard's pull of its usage | C — product | R6 | app 0.18.0, chart 0.20.0, report image at the same appVersion | [#67](https://github.com/ephico2real2/group-sync-dashboard/issues/67) | released |
| D2 | [`SPEC_D2_per_cluster_authorization.md`](SPEC_D2_per_cluster_authorization.md) — per-cluster authorization for the multi-cluster case | D — architecture | R7 | app 0.19.0, chart 0.21.0 | [#68](https://github.com/ephico2real2/group-sync-dashboard/issues/68) | released |
| E1 | [`SPEC_E1_report_library.md`](SPEC_E1_report_library.md) — the report library tab: one section per configured report, a run's expiry from the service | E — after the programme | — | app 0.25.0 | [#229](https://github.com/ephico2real2/group-sync-dashboard/issues/229) | merged |
| S1 | [`SPEC_S1_cluster_secrets.md`](SPEC_S1_cluster_secrets.md) — clusters as labelled Secrets: the contract, the reader, the API | S — cluster configuration | — | chart 0.42.0, migration 19 | [#230](https://github.com/ephico2real2/group-sync-dashboard/issues/230) | merged |
| S2 | [`SPEC_S2_cluster_configurations_tab.md`](SPEC_S2_cluster_configurations_tab.md) — the Cluster Configurations tab: the writes, the form and its GitOps twin | S — cluster configuration | — | chart 0.44.0 | [#230](https://github.com/ephico2real2/group-sync-dashboard/issues/230) | merged |
| S3 | [`SPEC_S3_connection_modes.md`](SPEC_S3_connection_modes.md) — connection modes: the dashboard obtains a remote cluster's credential (SATokenLookup, userSelfLogin) | S — cluster configuration | — | chart 0.47.0 (§4) and 0.49.0 (§3.1); the rest at its PR | [#230](https://github.com/ephico2real2/group-sync-dashboard/issues/230) | in progress |
...
| S4 | [`SPEC_S4_token_retrieval.md`](SPEC_S4_token_retrieval.md) — retrieving a cluster credential as the fleet account: the challenging-client login, the ServiceAccount token read, the write, and the lifecycle | S — cluster configuration | — | app and chart bumps per step, assigned at each step's PR | [#283](https://github.com/ephico2real2/group-sync-dashboard/issues/283) | in progress |
| S4a | [`SPEC_S4a_fleet_login_session.md`](SPEC_S4a_fleet_login_session.md) — S4 step A: the fleet login session — obtain, expire, retry, revoke; the code of #283 | S — cluster configuration | — | no version change (a module nothing calls yet; the release that first calls it bumps) | [#283](https://github.com/ephico2real2/group-sync-dashboard/issues/283) | merged |
| S4b | [`SPEC_S4b_sa_token_lookup.md`](SPEC_S4b_sa_token_lookup.md) — S4 step B: the ServiceAccount token lookup — read as the fleet account, write as the dashboard; the code of #284 | S — cluster configuration | — | app 0.31.0, chart 0.50.0 | [#284](https://github.com/ephico2real2/group-sync-dashboard/issues/284) | merged |
...
| S5 | [`SPEC_S5_configmap_onboarding.md`](SPEC_S5_configmap_onboarding.md) — ConfigMap onboarding: credential-free cluster declarations, remote lookup and owned Secret reconciliation | S — cluster configuration | — | app 0.34.0, chart 0.55.0 | [#293](https://github.com/ephico2real2/group-sync-dashboard/issues/293) | merged |
| L1 | [`SPEC_L1_remove_oauth_debug.md`](SPEC_L1_remove_oauth_debug.md) — Remove the OAuth Debug reader and authLogLevel Jobs; retain stored history and audit capture | L — login capture | — | app 0.36.0, chart 0.58.0 | [#321](https://github.com/ephico2real2/group-sync-dashboard/issues/321) | in progress |
| T2 | [`SPEC_T2_cluster_admin_tier.md`](SPEC_T2_cluster_admin_tier.md) — the cluster-admin tier: `visibility.clusterAdminSar` (`update clusterrolebindings` on the host) gates the KPI page and the whole Cluster Configurations tab, and grants every host tier to whoever passes it; replaces #230's two-level pair | T — tiers | — | app 0.35.0, chart 0.57.0 | [#322](https://github.com/ephico2real2/group-sync-dashboard/issues/322) | merged |
...
1. **Schema migrations.** B4, C2, C3 and D1 each claim the next migration number. Numbered in
   release order: B4 = 8, C2 = 9, D1 = 10, C3 = 11 (creates `cluster_namespace`,
   `cluster_namespace_status` and `report_run`). B2 adds indexes only, in `SCHEMA`, and needs
   no migration.
