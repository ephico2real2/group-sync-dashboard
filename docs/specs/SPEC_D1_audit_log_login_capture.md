...
- **Grounded in the reference cluster's real audit log (2026-09-05, operator request: "google the OpenShift audit log format and review the actual audit log here; no noise; capture users and the user id that matches the configured authentication or a discovered identity; we don't care about system accounts").** Read with `oc adm node-logs --role=master --path=oauth-server/audit.log` on CRC (audit profile `Default`): 49,360 records, 27.7 MB, spanning 2025-04-19 to 2026-09-05, every record `audit.k8s.io/v1 Event`, level `Metadata`, two stages per request (`RequestReceived`, `ResponseComplete`), so 24,680 requests. **Noise, measured:** 23,666 of those requests carry no `authentication.openshift.io/decision` at all — 23,218 are `HEAD /` and 2 are `GET /` (the proxy's health probes and two browser hits, `system:anonymous`, `authorization.k8s.io/decision: forbid`), the rest `/oauth/token`, `/favicon.ico`, logouts. Only **1,014 requests carry a decision**, and only **427 carry a username**. The `user` field is `system:anonymous` on every one of them; the person is only in the annotation, as Red Hat's own article states ([How to classify OpenShift audit logs](https://developers.redhat.com/articles/2024/07/29/how-classify-red-hat-openshift-audit-logs)); the documented values are `allow`, `deny`, `error` ([Viewing audit logs, 4.17](https://docs.redhat.com/en/documentation/openshift_container_platform/4.17/html/security_and_compliance/audit-log-view)). The 587 `deny` records **without** a username are 583 unauthenticated `GET /oauth/authorize` that precede every login page (statuses 200/302/401) and 4 consent-page requests — not failed logins, and not rows. **The event shapes that carry a username, by verb, path, decision, count:** `POST /login/<idp>` allow 91 (`developer` 47, `ldap-local` 44) and deny 21 — the interactive credential login, the only shape that names the identity provider; `POST /login` allow 34 / deny 1 — the same form on an older path with no provider in it (all 2026-08-03, kubeadmin; provider resolved through the Identity object); `GET /oauth/authorize` allow 240 with a username — two different things: `client_id=openshift-challenging-client` (107: `oc login` and curl basic-auth challenges — a credential login with no `/login` POST at all) and browser `client_id`s (133: the dashboard's own ServiceAccount client 115, `console` 16, GitOps 2 — an existing session re-authorising to a client, **not** a new login); `GET /oauth/authorize` deny 19 with a username, all `openshift-challenging-client` with status 401 — failed CLI logins, including `LATEEF.O` (case) and `nosuchperson`; `/oauth/authorize/approve` allow 21 — consent, not login. **Consequence for the parser (D1.3 `parse_audit_line`), applied at implementation:** (1) a row is written only for (a) `POST /login[/<idp>]` with a username, decision allow → success, deny → failure, and (b) `GET /oauth/authorize` with a username **and** `client_id=openshift-challenging-client` → CLI success/failure with provider resolved from the Identity object; (c) `GET /oauth/authorize` with a browser client_id and a username is a **session re-authorisation**, not a credential login: it is recorded as its own kind, `session`, with the `client_id` on the row (operator, 2026-09-05: "it is also okay to capture logins that originated from the group sync dashboard" — 115 of the 133 here are the dashboard's own proxy re-authorising, 16 the console, 2 GitOps), so the design's coalescing of the browser pair (D1.3 `_coalesce`, `ATTEMPT_WINDOW` = 1 s) is replaced by kinds — measured on all 133 browser re-authorisations, every one has an earlier credential allow for the same username, and the gap from the latest one is within 1 s for 77, within 2 s for 119, within 4 s for 125, and at most 20.97 s, so a 1 s window would have hidden 77 and left 56 as spurious logins (review of the note, Codex): `credential` for `/login`, `cli` for the challenging client, `session` for every other client — the Logins tab and `/logins` default to `credential` and `cli` and show `session` on request; (2) records without a username are never rows; (3) **every attempt is a row, and the identity is classified, not filtered** (operator, 2026-09-05: "we need to track all login failures as well, for all attempts, and capture the error"): the row carries `identity_match` — the configured provider the username resolves to through its `Identity` object (`loginCapture.auditLog.providers`, default: every provider configured on `oauth.config.openshift.io/cluster` — here `developer` (HTPasswd) and `ldap-local` (LDAP)), matched case-insensitively because a failed attempt can arrive in another case (`LATEEF.O` was a CLI deny for the person `lateef.o`; the provider refused it, and the row must still be attributed to that person — review of the note, Codex), or `null` for a name that resolves to nothing (`nosuchperson`, `ghost.user`, `testuser`, the typo `kubadmin`) — so a failed attempt against a name that does not exist is kept, visibly unmatched, and `gsd_login_capture_unmatched_total{cluster,decision}` counts them as well; **the error captured is what the record carries**, measured on all 41 failures: `decision` (`deny`; `error` appears only when the provider itself fails — zero here), `responseStatus.code` (302 for every browser failure, which is the redirect back to the form and carries **no reason**; 401 for every CLI failure) and `responseStatus.message` when present (`Authentication failed, attempted: basic` on every CLI failure, absent on browser failures), plus the provider or client and the user-agent family — there is no richer failure text in the audit log at any profile; the reason strings the Debug pod log carried ("failed for … : invalid credentials") do not exist here, which the Logins tab says in its failure column's empty state instead of inventing one; (4) **system accounts:** `system:*` and `kube:admin` are never rows, and `loginCapture.auditLog.ignoreIdentityPatterns` (default `["ou=TrustedApplications"]`, matched against the decoded LDAP DN in the Identity's `providerUserName`) drops service identities — measured: `ocp-oauth-bind-serviceid` produced 4 allows and 1 deny through the challenging client on `ldap-local`, and the exclusion drops **all** of them, the failure included (a service identity's failures are not personnel events either); its Identity name decodes to `cn=ocp-oauth-bind-serviceid,ou=TrustedApplications,dc=…`, so the OU is the honest, data-derived discriminator rather than a name list — the pattern is matched case-insensitively against **both** the Identity's `providerUserName` as-is (for the LDAP provider that field is the raw DN; for HTPasswd it is the plain username, which can never match an `ou=`) and the base64url-decoded suffix of the Identity's name after the first `:` (review of the note, Cursor: decoding `providerUserName` would fail on a raw DN and let the service identity through); (5) the values for these keys (`providers`, `ignoreIdentityPatterns`), the `kind`, `client_id`, `identity_match`, `status_code` and `error_message` columns (migration 10 grows accordingly), the `/logins?kind=` filter, the `gsd_login_capture_unmatched_total{cluster,decision}` family (D1.8's list gains it) and their tests join the body at implementation, with this note as their source. **Body passages this note supersedes** (review of the note, Cursor — the body still tells the reader the opposite about CLI logins): D1.3 "WHAT IT CANNOT SAY" — a `deny` carries no cause (the LDAP result codes exist only in the Debug pod log), so a deny is `failed` and `detail` says so; the identity provider is the `/login/<idp>` path when present and otherwise the User's Identity provider, so a CLI `kubeadmin` row carries `provider=developer` and **can** be labelled break-glass through `loginCapture.htpasswdProviders`; D1.7's `/logins` note, D1.9's values comment, the chart README's `htpasswdProviders` row and the changelog lose the sentence "names the identity provider only for browser logins, so a CLI kubeadmin login recorded from it is not labelled break-glass" and gain the sentence above; D1.10's `test_the_provider_is_read_from_the_browser_path_only` is replaced by a test that a challenging-client login's provider is resolved from the Identity (`developer:kubeadmin` → `developer`); `_coalesce` and `record_audit_login_events` check 3 and `test_a_browser_logins_second_request_is_one_attempt` are replaced by the `kind` classification (`credential` / `cli` / `session`) — no coalescing, every annotated request with a username **of the three admitted shapes** is its own row of its kind — `credential` (`POST /login[/<idp>]`), `cli` (`GET /oauth/authorize` with `client_id=openshift-challenging-client`) and `session` (`GET /oauth/authorize` with any other client). The 21 username-bearing `GET|POST /oauth/authorize/approve` records are consent decisions, not logins, and are ignored (review of the note, Codex: "every annotated record" and "consent is not a login" contradicted each other; the rule is the three shapes). **The query string:** D1.3 keeps the path without its query and says `client_id` is not stored; for `cli` and `session` rows the parser now extracts and persists **only** `client_id` from the query and discards `redirect_uri`, `state`, PKCE parameters and everything else — the discriminator is kept, the noise is not. D1.8 and the changelog name **three** new families, the third being `gsd_login_capture_unmatched_total{cluster,decision}`; the body's "two families" is superseded. Tests the review supplied for R5, in the body's `_event`/`parse_audit_line`/`audit_event_dict` vocabulary: a credential POST followed 300 ms later by a dashboard-client authorize yields two rows of kinds `credential` and `session` with the `client_id` on the second and both stored (`record_audit_login_events` returns `(2, 0)`); a `session` row's JSON carries the `client_id` and no `redirect_uri`, `state` or `code_challenge` text; `GET` and `POST /oauth/authorize/approve` parse to `None`; `identity_match_for("ALICE.EXAMPLE", identities=[…uid=alice.example…], configured_providers={"developer","ldap-local"})` returns `ldap-local`. The design's unverified row (the table's "could not verify" on whether a browser login produces one or two annotated events) is now **measured: two** (POST `/login/<idp>` then GET `/oauth/authorize`, both allow with the username, 1–2 s apart), and the rule in (1) makes the second one irrelevant rather than coalesced. Also measured: Red Hat's solution 5810281 confirms that below Debug the pod log carries no login lines and that the audit log is the supported source ([User login attempts not found in the audit log files](https://access.redhat.com/solutions/5810281)); the reference cluster's Identity objects span providers `ceo_rnd_oim` (50), `ldap-local` (8), `developer` (2), `my-provider` (2), `pnc_rnd_oim` (2) — the first, fourth and fifth are not configured on the OAuth CR any more, which is exactly why the filter is "configured providers" with the discovered Identity as the join, not the Identity list alone.
- Applied at implementation (PR for #66), 2026-09-06, from the body's blocks as amended by the grounding note above. Deviations, each with the reason: (1) **file identity is a head fingerprint, not the size.** Measured on the reference cluster (audit.log of 27,804,760 bytes): `Range: bytes=<size>-` — the idle case, nothing new since the last read — answers `416` with `Content-Range: bytes */<size>`, exactly as a Range past the end does, and `HEAD` answers `405`. The body's reader took every 416 as rotation, which would have reset audit.log's cursor on every idle cycle; and its loop visited a fresh rotated file (sorted before `audit.log`) before noticing the rotation, so the hand-over never fired and the rotated file was re-read from byte 0 — while the size alone cannot tell the file that was read from a new `audit.log` that has already grown past the cursor (a resume would start mid-line in the new file and lose its first bytes). As built: `kube.py#fetch_node_log_file` decides a 416 by the size in `Content-Range` (equal to the cursor is "nothing new", below it is `rotated`; absent, one unranged read settles it the 200 way); each cursor row carries `head_sha256`/`head_len`, the sha256 of the file's first `min(1024, bytes consumed)` bytes (`auditlog.py#FINGERPRINT_BYTES`, at least one whole line with its unique `auditID` — Filebeat's `file_identity.fingerprint`), re-read and compared before every resume; `audit.log` is visited first once it has a cursor, and `auditlog.py#_hand_over` moves the cursor and fingerprint to the newest cursor-less rotated file only after that file's head matches, updating the in-cycle view so the rotated file resumes from the handed-over offset in the same cycle. One extra ~1 KiB request per in-progress file per cycle; complete rotated files pay nothing. Tests: `test_kube_reader.py::TestNodeLogRangeReads` (the measured 416 forms), `test_auditlog.py` rotation past the cursor, the idle cycle, a rotated file whose head changed. (2) The fixtures in `tests/test_auditlog.py` are the eleven measured records the grounding note counted, anonymised (`userN.example`) and otherwise byte-identical, one keeping the `crc ` node-name prefix `oc adm node-logs --role` writes; the batch test that reads them runs with `loginRetentionDays: 0` because the cluster's records are older than the 400-day default — two of the eleven were inside it and nine were, correctly, aged out. (3) The kinds, the identity classification (`identity_match`, `loginCapture.auditLog.providers`, `ignoreIdentityPatterns`), the absence of coalescing, the `gsd_login_capture_unmatched_total{cluster,decision}` counter and the `kind` query parameter are the grounding note's, not the body's; the body's coalescing paragraph in the design doc is replaced by the note's rule. (4) Schema migration 10, and the design doc heading names application 0.17.0; the body's 0.13.0 / 0.12.0 pairs and its CHANGELOG heading are superseded by the ladder and by the heading `prepare-release.py` writes. (5) The chart README's `htpasswdProviders` row says a CLI `kubeadmin` login IS labelled break-glass (the provider resolves through the User's Identity) where the body said it is not; the values comment says the same. (6) `docs/LOGIN_CAPTURE_QUICKCHECK.md` §6 states the measured answer, `206`, for the ranged read, and the Node column replaces Replica on the Logins tab only when the envelope's `source` is `audit-log`. (7) `ClusterError` is not imported by the test module (unused in the body's tests). (8) The audit `detail` names the request path as the target for credential logins (`via /login/<idp>`), the client for CLI and session ones.
