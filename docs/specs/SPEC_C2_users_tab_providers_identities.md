...
- Deviations from the adversarial review (`docs/REVIEW_C2.md`), applied in the same PR: (7) `fetch_identities` compares Identity times as instants (`datetime.fromisoformat`), not strings — the API server has marshalled both second and microsecond precision, and a string minimum between the two widths picks the later instant; an unparsable stamp is skipped; (8) a transient failure of the Identity read (or a client without the method) keeps the store's last-known exact times (`poller._last_known_exact`) instead of rewriting every row to the User time while `identities_source` still said `ok`, which made the tab's note say "exact" over chips saying "approx."; (9) `_providers_setting` refuses exactly what the OAuth CR refuses (`oc explain oauth.spec.identityProviders.name`: a path segment, not `.` or `..`, no `/`, `%` or `:`), plus whitespace; the review's DNS-1123 proposal was rejected because the API accepts upper case and underscores; (10) the lookup-mapping caveat is stated in the method's docstring and the design doc; (11) `local-development/API.md`'s users section carries the new envelope and row fields, held by a contract test; (12) the migration-9 test bounds the latest version with `>=`, and `config.users` sits after `unmanagedAudit` rather than inside the `userActivity` comment block.
