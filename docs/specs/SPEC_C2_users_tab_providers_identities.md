...
| Version on release | app 0.15.0, chart 0.16.0 |
...
- Second feature of R3: app 0.15.0, chart 0.16.0; schema migration 9 (the body says 8; B4 took 8).
- Applied at implementation (PR for #62). Deviations and fillings-in, each because the body describes rather than writes: (1) the body's "### Tests" section is prose, so the test code was written from its descriptions — `tests/test_identities_read.py` (paging with a continue token, the earliest of two Identities, an orphan skipped, 403 → None, 503 raises; the poller matrix off / ok / forbidden / transient), `TestExactFirstLoginAndTheProviderAllowList` in `tests/test_users_tab_logins.py`, `TestUsersProvidersAndIdentitiesRead` in `tests/test_config.py`, `TestTheIdentitiesGrantIsOffReadOnlyAndCoupled` in `tests/test_chart_strategy.py`, `TestExactFirstLogin` and the `providers_server` fixture in `tests/test_ui.py`, and `test_migration_9_…` in `tests/test_migrations.py`; (2) `_client` in `tests/test_users_tab_logins.py` gained a `settings_extra` mapping so a test can set `users_providers` or `identities_read_enabled` without a second helper; (3) two existing expectations followed the seed the body prescribes: alice's `first_login_at` is now her Identity time (29 days) rather than the User time (30 days), and the C1 export header list carries `first_login_source` (the body's own `EXPORT_COLUMNS.users` edit); (4) the migration-8 test's `== 8` on the latest version became `>= 8`, since 9 now exists; (5) the values comment on `rbac.identities` says the 0.14.0 rule (RBAC-costing switches stay off) is why it defaults false; (6) the body's docs bullets cite `Application 0.14.0 — chart 0.13.0`; the ladder's pair (app 0.15.0, chart 0.16.0) is used, and `docs/DESIGN_users_tab_logins.md` gains the "Decisions after 0.9.0" section as described.
- Deviations from the adversarial review (`docs/REVIEW_C2.md`), applied in the same PR: (7) `fetch_identities` compares Identity times as instants (`datetime.fromisoformat`), not strings; proposed by Cursor for a mixed-width reason Codex refuted at the source (apimachinery's `Time.MarshalJSON` writes fixed-width RFC3339 seconds), kept as the operation the code means, which holds at any width or offset; an unparsable stamp is skipped; (8) a transient failure of the Identity read (or a client without the method) keeps the store's last-known Identity times (`poller._last_known_identity_times`) instead of rewriting every row to the User time while `identities_source` still said `ok`; a User who appears during such a cycle carries the User time until the next successful read, and the tab's `ok` note describes the source per row rather than asserting one for all (Codex measured the mixed snapshot; its proposed `error` state, which would downgrade every row on one 503, was rejected); (9) `_providers_setting` refuses exactly what the OAuth CR refuses (`oc explain oauth.spec.identityProviders.name`: a path segment, not `.` or `..`, no `/`, `%` or `:`) and nothing more — Cursor's DNS-1123 proposal was rejected because the API accepts upper case and underscores, and the whitespace refusal added then was removed after Codex showed spaces and commas are legal names; so that every legal name travels, the chart renders `config.users.providers` as a YAML flow sequence (`toJson`) rather than a comma-joined string, and the app accepts a list (the comma form remains for `GSD_USERS_PROVIDERS` and hand-written files, where a name containing a comma cannot be expressed); (10) the lookup-mapping caveat is stated in the method's docstring and the design doc; (11) `local-development/API.md`'s users section carries the new envelope and row fields, held by a contract test; (12) the migration-9 test bounds the latest version with `>=`, and `config.users` sits after `unmanagedAudit` rather than inside the `userActivity` comment block; (13) files outside the body's list, all bookkeeping: `docs/specs/README.md` and `docs/specs/SPEC_C1_table_export.md` mark C1 released, `docs/REVIEW_C2.md` records this review and `local-development/tests/test_docs_citations.py` registers it, `local-development/tests/test_values_defaults.py` lists `rbac.identities` among the guarded off-by-default switches, and `docs/DESIGN_users_tab_logins.md`'s opening sentence, mangled by the first edit, was restored; (14) the Identity time is never called "exact" (Codex's principal finding): with `mappingMethod: lookup` an administrator creates the Identity before the first login, so the chip reads `identity` with the caveat in its title, the `ok` note states it, and the CHANGELOG, chart README, values comment, API.md and the specs index say "first login from Identity objects" — the mechanism the body specifies is unchanged, the label is honest; Codex's alternative (never promote the Identity time) was rejected because it removes the feature the issue asks for while the time is the first login for the default mapping methods.
