...
- Routed here from the chart 0.14.0 review (Codex, second pass, 2026-09-05): `gsd/kube.py` `TierResolver._resolve_and_cache` adds only `VIRTUAL_AUTH_GROUPS` (`system:authenticated`, `system:authenticated:oauth`) to the repeated SubjectAccessReview, so a ServiceAccount identity (`system:serviceaccount:<ns>:<name>`) granted through `system:serviceaccounts` or `system:serviceaccounts:<ns>` passes the proxy's review and resolves to the self tier in the application. Kubernetes gives ServiceAccounts `system:serviceaccounts` and `system:serviceaccounts:<namespace>` (apiserver `pkg/authentication/serviceaccount`). When this spec rebuilds the resolver per cluster, derive the virtual groups from the identity's shape (a `_virtual_groups_for(viewer)` helper) and test a namespace-group grant resolving to the wide tier; until then `docs/api-access.md` tells operators to grant the ServiceAccount in the user form.
- Routed here from the C1 review (Codex, second pass, 2026-09-05): `index.html` `refresh()` chooses the narrowed reader's `/users/<name>` request from the PREVIOUS cycle's `data.whoami` before the new whoami arrives; if the proxy session changes user while a page stays open (Alice's tab becomes Bob's session), the speculative request is `/users/alice` as Bob, the API answers 403 ("user profiles other than your own need the wide tier" — measured in memory), the shared `Promise.all` rejects, and the follow-up that would re-fetch the new viewer's own path never runs — the page shows the generic API error until a reload. Codex's fix: a `myAccessFetch(get, base, viewer, {allowStaleViewer})` helper that returns null for a 403 on the speculative call only, the speculative call made only when `data.whoami.user` is known, and the narrowed follow-up re-fetching when `data.myAccess.viewer !== w.user`; with a Playwright test that switches `X-Forwarded-User` from alice to bob on an open page, calls `refresh()`, and expects `data.myAccess.viewer == "bob"`, no API error card, and an export of scope `self`, viewer `bob`, zero rows. Pre-existing (not introduced by C1) and owned by this spec's rebuild of the reader-tier flow; apply it here.
...
- Applied at implementation (PR for #68), 2026-09-13, from the body's blocks: the 43 fenced blocks extracted mechanically from their fences and applied with an exact-match check on every Old text — all 47 edits matched the tree seven releases after the design was written. Deviations, each with the reason: (1) **versions and dates are the ladder's**: app 0.19.0 / chart 0.21.0 / 2026-09-13 wherever the body says 0.12.0 / 0.11.0 / 2026-09-04 (the chart README rows, the CHANGELOG bullets, the test docstring, `docs/SPEC_per_user_visibility.md`, `README.md`); the body's `Chart.yaml` history paragraphs (D2.5) are replaced by `prepare-release.py`'s two-line history and the CHANGELOG entry carries the explanation, as every release since A3. (2) **The `list_alerts` block predates B4**: the body's replacement dropped `st.cliff_policy(settings)` and the `group_count_changes=` / `cliff=` arguments to `compute_alerts` that B4 (0.12.0) added inside the replaced range — `tests/test_view_scoping.py`'s cliff test found it; both are restored in the per-cluster loop. (3) **`tests/test_visibility.py` needed three edits**, not none: its second cluster `c2` now says `visibility="inherit"` (the fixture change D2.6 itself names, because a second cluster is self-only by default); `/api/clusters` rows and `/api/whoami` now carry a per-reader decision, so the row equality strips `visibility` and asserts the scopes, and the whoami assertion checks the two headline keys plus `clusters`; `tests/test_activity.py`'s exact-equality whoami body — written so that a new key comes to it for a ruling — gains `"clusters": {}`. (4) **`test_notes_name_every_clusters_policy` renders through `tests/test_chart_route.py#_notes_probe_chart`**: `helm template` drops `NOTES.txt`, so the body's plain render could never have seen the text. (5) **The two routed items are applied here**: `gsd/kube.py#_virtual_groups_for` derives a ServiceAccount's `system:serviceaccounts`, `system:serviceaccounts:<ns>` and `system:authenticated` from the identity's shape (an OAuth token's `system:authenticated:oauth` is not among them), with tests in `tests/test_visibility_tier.py` and `docs/api-access.md` updated; `index.html`'s `myAccessFetch` takes `allowStaleViewer` (the speculative call resolves a 403 to null) and the narrowed follow-up refetches whenever the held path is missing or names another viewer, with the Playwright test in `tests/test_ui.py`. (6) D2.7's prose instructions are written as `docs/ACCESS_CONTROL.md` §11 and the §5 diagram lines, the two `reference-architecture.md` replacements, the `README.md` paragraph and the dated line in `SPEC_per_user_visibility.md`. (7) The two operator questions (one identity provider across the fleet; remote `create subjectaccessreviews`) were unanswered when the PR opened; the body's defaults — `identity: none` and `remote-sar` opt-in — are the safe answers to both and are taken, posted on #68. (8) **Found by the live check on the reference cluster:** `gsd.validateClusters` died on a Go nil pointer ("nil pointer evaluating interface {}.name") when a second cluster was added with `--set clusters[1].…` on a release whose values file does not define `clusters[0]` — Helm never merges lists and pads an index set beyond the list's length with `null`, so the merged list was `[null, {…}]`; the guard now refuses a non-map entry by index with a sentence that says so and names the remedy (pass every entry, `clusters[0]` included), and a chart test pins the padded case. (9) **From the adversarial review, first pass (`docs/REVIEW_D2.md`).** Cursor: `inherit` meant the host's RESOLVER, not the host's DECIDED tier — a `self-only` host (legal per the guard) with an `inherit` remote served the remote wide and the whoami headline said `all` above a host row that said `self` (measured); `viewer_scope` now resolves a nameless question to the host cluster's own policy and an `inherit` remote to the host's policy, so a self-only host is self on every cluster it governs; the Reports tab reads the host's headline (`narrowedOnHost`), not the selected cluster, because the ticket is minted on the host's tier. Codex: a blank or whitespace `visibility`/`identity` rendered through the chart's guard and crashed the pod at startup — `load_settings` now treats them as unset; the hand-built poll-failure and dangling-binding alerts had lost `silenced`/`silenced_by` (the body's block predated B4, like deviation 2) — restored; the header pill tested `=== "self"` while every narrowing helper tests `!== "all"` — now the same fail-closed rule. Both: NOTES took index 0 as the host where the guard and `load_settings` take the first ENABLED entry, and did not trim — rewritten to find the host and print it first; the ConfigMap had gained two template comment lines INSIDE the config the pod reads, so the default 0.20.0 → 0.21.0 render was not byte-identical — removed; the body's `test_the_default_single_cluster_render_is_unchanged` ended in `or True` and could not fail — rewritten; §11 named `/api/events`, which does not exist — corrected, and a test holds every route the section names to the app's route table; "first entry" is "first enabled entry" in the values comment, the chart README, the changelog and §11; the values comment said "chart 0.10.x" (the body's ladder) — 0.20.x. Also: `helm.yaml`'s label-step error claimed "the chart release itself is unaffected and will still publish" while chart-releaser has no `if:` and never runs after the step's `exit 1` — the message now says the job stops (Codex, C12). **Rejected, with the reason:** byte-identical 404 bodies for hidden and unknown ids (Codex F1) — the body differs only by the id the caller sent, which is the caller's own input and not information about the server, and the spec's own test asserts exactly that; DNS-1123 validation of a ServiceAccount's namespace and name (Codex F2) — a name no apiserver issues yields a group no binding names, the review answers a denial either way, no hole; a 30-minute wait loop in `helm.yaml` for the image (Codex F7) — a cross-workflow timing assumption the file's own comment forbids after #37, and `validate` gates `release` so the chart run has trailed publish on every release since A2; the four prose-keyword doc tests, as on every PR. **Clarified, not changed:** under `inherit` identity is not consulted (the body withholds the viewer under `self-only` only), so an `inherit` remote with the default `identity: none` keys a self reader by the host's username, as before 0.19.0 — stated in §11's table and the values comment. (10) **Second pass (`docs/REVIEW_D2.md`).** Cursor: the pass-1 remap of `inherit` onto the host's policy then applied the REMOTE's default `identity: none` and withheld the viewer — the cluster's own policy now decides whether identity is consulted (kept under inherit), and with no enabled cluster at all the nameless question fails closed instead of falling through to the host resolver; whoami and alerts decided the host twice (nameless, then as a row) — the headline is the host row's decision and the alerts envelope takes its viewer from `trusted_viewer`; the cluster selector still tested `=== "self"` — `!== "all"` like the pill; the Reports fetch guard is pinned by a second Playwright test; the CHANGELOG's upgrade note says to pass every entry when adding one with `--set`. Rejected: rendering the policy on the Overview's cluster card (the brief overreached; the spec marks a narrowed cluster in the selector and the pill). Codex (its most important finding): a quoted `enabled: "false"` in a values file — a non-empty string, truthy in Go and in `bool()` — made the disabled entry the HOST in the guard, NOTES and `load_settings` alike (measured); `enabled` is now a word in all three (a boolean or the strings true/false; anything else refused by name), with the chart halves tested in the chart module and the application half in `tests/test_config.py` (CI's chart job runs without the application — C3 deviation 10's lesson, met again on the first CI run of this fix); §5's decision diagram cited line numbers (the `file:NNN` form the citation rule forbids) hundreds of lines stale inside a code block the rule does not scan — anchors now, held to definitions by a test; `refresh()` chose the Reports requests on the previous cycle's host tier, so a reader promoted mid-session saw "Loading…" for a cycle — the Access granted tab's catch-up, applied to Reports, with a Playwright test. Rejected: a byte-identical test that renders `git archive main` at test time (a checkout without `main` cannot run it); a `narrowedScope()` refactor of the eight `=== "self"` comparisons on tab payloads (the API envelope's `scope` is a closed vocabulary, always present on a 200 — the fail-closed rule exists for whoami's `visibility.scope`, which the helpers read); the Overview card badge, as above.
