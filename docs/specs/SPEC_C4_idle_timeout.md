...
- Applied at implementation (PR for #65), 2026-09-06, from the body's blocks. Deviations, each with the reason: (1) `gsd.idleTimeout` ends in `| toYaml`: `include` returns TEXT, and the body's bare map printed as text is Go's `map[k:v]`, which `fromYaml` cannot read — every caller saw an empty map and the defaults, so `--set session.idleTimeout.*` was silently lost (four chart tests red at first run); (2) the body's `_idle_timeout_setting` is inserted before the comment block that precedes `_SAR_FIELD_PATTERNS` (the body's anchor assumed two blank lines there); (3) `tests/test_ui.py::test_the_poll_is_suspended_while_the_warning_is_up_and_resumes_after_stay` resets its fetch counter once the warning is up: the nine legitimate polls before the warning were being counted as "the poll ran under the countdown"; (4) the body's CHANGELOG and Chart.yaml texts name 0.13.0 / 0.12.0 — the ladder's pair is used, and the changelog heading is the one `prepare-release.py` writes; (5) the design doc's rewrite is authored from the body's prose (it describes the section rather than writing it) and cites the helper, the setting function, `idleExpire` and `autoRefresh`; (6) `session.idleTimeout.enabled` joins `tests/test_values_defaults.py`'s stated off-by-default exceptions with its reason; (7) the body's four closing questions are batch C's and were settled on 2026-09-05 (exports not recorded; the C3 marking, PDF base and own-namespace questions in `SPEC_C3_reporting_microservice.md` §14). (8) From the adversarial review (`docs/REVIEW_C4.md`, Cursor): expiry is final and complete — "Sign out now" goes through `idleExpire` (the body only set the state, leaving the rows on screen and Escape able to revive the session), `idleStay` refuses once expired, `idleExpire` bumps `navSeq` and `refresh()` returns when expired at its start, before it paints and in its error path (a refresh in flight could otherwise repaint rows, or the cap's "session has ended" panel, over the inactivity card — and every refresh fetches whoami, the re-stamp lesson 3 exists to stop), `popstate` is gated while the countdown is up, and the skip link — a sibling of `#wrap` — is made inert with it; `_idle_timeout_setting` reads minutes as whole-number text (a YAML `1.5` truncated to 1 while the same value from the environment fell back to 30) and treats a cap of 0 or below as no cap; `tests/test_activity.py`'s exact whoami body and `API.md`'s example gain `idle_timeout` (the exact test was red on the first head); the values comment, README row and CHANGELOG say what `minutes` is — the whole window before sign-out, the countdown being its last `warningSeconds` — where the body's Goal sentence had it as the time before the countdown. Live on CRC with a one-minute window: the dialog at 29.4 s, expiry at 59.4 s landing on `/signed-out` via the proxy's `sign_out`, re-entry demanding credentials. (9) From Codex's pass of the same review (on the pre-Cursor head; its overlapping findings — the expiry race, "Sign out now", history navigation, the skip link, the exact whoami consumer — were already applied): `hashchange` is gated like `popstate`; `refresh()`'s error path also returns when superseded; both idle numbers are read by `_idle_integer_setting`, which refuses YAML booleans and floats as text (`int(True)` and `int(1.5)` were both a one-minute window); `gsd.cookieExpire` returns a numeric `0` as typed instead of Helm's `default` swallowing it into `4h`, and the deployment refuses `0` by name (a zero cap is not a cap) — measured: `--set oauthProxy.cookie.expire=0` rendered `-cookie-expire=4h`; the design doc, the whoami docstring and the page's session comment no longer say whoami is "never polled" (every data refresh fetches it, for `visibility`; there is no session-only timer, and the automatic refresh is suspended under the countdown) nor that the sign-out link is an app-side `/sign-out` that revokes the token; the chart README's `skipAuthRegex` row shows the current default. Codex's view that a cap of 0 or −1 is not a "no cap" value is recorded beside Cursor's: the app never receives either from the chart (`_duration_setting` falls back on a unit-less value), so the `> 0` guard is unreachable in practice and harmless. (10) Second pass (`docs/REVIEW_C4.md`, Cursor): `popstate` applies the position the browser moved to and skips only the `refresh()` while the countdown is up or expired — the first gate returned before `applyPosition`, leaving the view on one page under another page's URL; `API.md`'s whoami section and the values block comment carry the corrected wording ("never poll it" and "countdown after `minutes`" had survived the first pass); tests for Back applying the position, the header's Refresh after expiry, negative minutes and environment precedence. Cursor's file-contract tests on `API.md` and `values.yaml` were rejected as prose-asserting, like every earlier one. (11) Codex's second pass: the zero-cap refusal compares the RESOLVED seconds, not the typed text — `0s`, `0.0s` and `0h0m` rendered `-cookie-expire=0s` and friends past the `"0"` compare (measured); the chart test covers those spellings. Its findings on `popstate` and the two documentation blocks were the same as Cursor's second pass and were already fixed in the head it finished on. (12) Released by PR #87 (1b9f18b) as application 0.16.0 / chart 0.18.0; the publish run signed the digest and its aliases (`--preserve-digests`), and the chart's provenance verified from a laptop.
