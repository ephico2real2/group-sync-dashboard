...
| C3 | `credential.kind: oauth` refused with the operator's reason, before any I/O | CONFIRMED | CONFIRMED (`writer.py:97-99`, the disabled radio and its exact sentence) | Holds. |
...
| C8 | The writes switch defaults off; off means the routes are not registered | CONFIRMED | CONFIRMED (defaults at `config.py:474`, `values.yaml:1290-1291`; RBAC exact; mutations caught) | Holds. |
