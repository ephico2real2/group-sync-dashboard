...
- **The hover rail no longer paints under the first cell's text (#219).** Every `.rowlink` table's first column, header and cells together, keeps 3 px clear of the inset accent rail (the "− left" glyph's ink began at x = 1 under a rail spanning x = 0..2, measured on hover); `:where()` keeps the rule below the KPI page's wider padding, the risk rows' own rail and the report table's cells. A test holds the first cell's text ≥ 3 px in and the header aligned with it.
