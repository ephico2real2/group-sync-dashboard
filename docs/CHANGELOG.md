...
- **The KPI page's Observe door auto-discovers the console (#157).** `console.url` empty means discovered: the host cluster's poll thread reads `openshift-config-managed/console-public`, which a Role grants `get` on to every authenticated identity (measured with the pod's own ServiceAccount), and the door opens the console's **namespace-workloads dashboard scoped to the pod's own namespace** with the parameters the monitoring plugin reads (`project-dropdown-value`, `namespace`, `type=ALL_OPTION_KEY` — from its source). `environments/crc.yaml` turns the ServiceMonitor, the rules and the CR on and points `grafana.url` at the lab's own Grafana; both doors are live on CRC (`reports/2026-09-19_grafana-observe-162-161/`).
