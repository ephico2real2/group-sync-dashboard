...
- **Status text clears 4.5:1 on the rows it sits in, in every palette (#204).** `.change-added`, `.change-removed` and `td.num.warn` measured 4.01–4.44 on zebra and hovered rows in eight of ten theme × palette variants; every block's `--status-good`, `--status-critical` and light `--warn` move by the smallest hue-preserving step that clears the hovered row, the dark ones against the 14 % wash the dark sheet really paints (the accessibility fixture had modelled 10 % for both themes, and the review caught it). The light CVD palettes give warning text its own hue — the inherited amber sat 0.5° from their vermilion critical — held at the Okabe-Ito reference's own 14° between vermilion and orange; the KPI page's critical chip paints an opaque surface under its text instead of its translucent wash over a row wash (4.06–4.31 there). `--status-warning`, the 3:1 glyph amber, is untouched. The row guard adds the three tokens (31 failures on the previous sheet), a hue-separation test and the chip test.
