...
- **The hover rail no longer paints under the first cell's text (#219).** Every `table:has(tr.rowlink)` first column, header and cells together, keeps 3 px clear of the inset accent rail (the user-History "− left" glyph began at x = 1 under a rail spanning x = 0..2, measured on hover). `:where()` keeps the rule at `(0,1,1)`, so the KPI clusters table's `padding-left: var(--space-9)` (18 px, the same specificity, later) still wins; the Namespace-audit exposure table and the Reports catalogue carry no `.rowlink` rows and take no rule. Tests hold the Groups first cell and the user-History "− left" cell ≥ 3 px in, the KPI first cell at 18 px and the exposure table at its own padding.
