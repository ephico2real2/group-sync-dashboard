...
| `logLevel` | `INFO` | `DEBUG` adds this app's own reasoning: login-capture accounting per pod, poll timing, page counts, which replica holds the Lease. **Not** HTTP request lines — httpx logs those at `INFO` already. Not the same value as `authLogLevel` |
