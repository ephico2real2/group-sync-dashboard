...
| `kyverno.metricsUrl` | `""` | the three controllers' in-cluster `/metrics` endpoints, comma-separated | lab override — the Kyverno module's report breakers (#170): the lab runs Kyverno in `kyverno`, and the three circuits live on three endpoints; empty leaves the truncation state unknown |
