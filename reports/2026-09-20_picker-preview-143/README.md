...
Deployed head `44587ce` (`feat/143-picker-preview`, through the Argo Application; `running : 44587ce783 —
...
| the explicit names are a picker over the poll's namespaces | `picker  : 106 namespaces discovered on crc-local, first ['beta-prod', 'beta-rnd', 'beta-uat', 'ca-tutorial', 'ca-tutorial-app']` |
| type-ahead narrows it | `typeahead: 'group-sync' → ['group-sync-dashboard', 'group-sync-operator']` |
| a pick gives the totals of that run | `totals  : group-sync-dashboard → 1 namespaces · 0 group bindings · 0 user bindings` |
...
| 375 px | `no-x-overflow, picker options [True, 106]`; `Advanced open True | picker input top 120` |
