...
Deployed head `da46d4e` (`feat/170-kyverno-module` after the Grok/Codex pass — the grants `list` only, through the Argo
Application; `running : da46d4e72a — verified in-pod`; the first walk at `0989e4b` read the same lines), walked with `walk.py` through the OAuth proxy as kubeadmin, Chromium 1280×900 then
...
| claim | measured |
|---|---|
| the API's payload | `{"present": true, "api_group": "wgpolicyk8s.io/v1alpha2", "policy_kinds": [the five CEL kinds], "reports": 115, "legacy_results": 667, "other_results": 0, "breaker_total": 12749, "breaker_drops": null, "results": {"pass": 9, "fail": 0, …}, "policies": 1, "total": 0}` |
...
| the policy and its report counts | `[['ValidatingPolicy', 'restrict-nco-config-writers', 'on', 'off', 'Audit', 'Fail', 'yes · skip generating ValidatingAdmissionPolicy: not enabled.', '9', '0', '0', '0', '0']]` — the controller's own word beside Ready (finding 10's opt-in, on the page) |
