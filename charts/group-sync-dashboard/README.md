...
| `grafana.url` / `grafana.dashboardUid` | `""` / `""` | the "Open in Grafana" door on the KPI page renders only when the URL is set — Grafana is optional and operator-owned since OpenShift 4.11 removed the bundled one |
| `console.url` | `""` | the console route the KPI page's Observe → Dashboards link opens; omitted when empty |
