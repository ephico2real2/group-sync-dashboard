...
| `rbacAuditors.enabled` | `false` | opt-in. When true, renders a read-only auditor ClusterRole and a ClusterRoleBinding per configured group, so members can run reports and review users/groups/bindings in OpenShift directly — no workload access, no Usage tab. Off renders none of it |
...
| `rbacAuditors.groups` | `[]` | the groups to bind, each `{name, createLocal}`. Bound by name, so a synced group and a local group work the same. `createLocal: true` also creates a local `Group` object; leave it `false` for a group the group-sync operator owns |
