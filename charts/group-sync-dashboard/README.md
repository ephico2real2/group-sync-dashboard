...
| `reporting.retention.manual.days` / `.maxRuns` | `3` / `500` | on-demand (manual) run retention, whichever bound is hit first; `0` disables a bound. Both tiers age a run from its **completion** (`finished_at`), not its request, and "newest" means most recently completed (#163) | a run that waited is as old as its file, not its ticket |
