...
def parse_secret(obj: dict, *, host_name: str | None) -> ClusterConfig | Finding:
    ...
    if visibility is not None and visibility not in allowed:
        ...
        # nobody may see wide"), which would make remote-sar silently mean self-only. S2 builds them
        # at discovery; until then the Secret says inherit, self-only or hidden.
