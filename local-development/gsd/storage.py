...
class StorageBackend(Protocol):
    ...
    # `rows=None` above and this reader are a pair: None records "the GroupSync CRD is not
    # installed", which zero rows cannot express. Three-valued — None here means "never
    # polled" — so a backend that cannot distinguish the three states does not satisfy this.
    ...
    def replace_users(
        self, cluster_id: str, users: list[dict], observed_at: str,
        identity_created: dict[str, str] | None = None,
    ) -> None: ...
    ...
    # -- the KPI module (#156): scalars, the event watermarks and the daily rollup ----------------
    ...
    def namespaces(
        self, cluster_id: str, *, user_name: str | None = None, groups: list[str] | None = None,
        every: bool = False,
    ) -> list[dict]: ...
    def namespace_reach(self, cluster_id: str, name: str, user_name: str, groups: list[str]) -> bool: ...
    def namespace_detail(
        self, cluster_id: str, name: str, *, user_name: str | None = None,
        groups: list[str] | None = None, sibling_key: str | None = None,
    ) -> dict: ...
