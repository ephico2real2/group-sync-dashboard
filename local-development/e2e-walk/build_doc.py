...
disk. Output: e2e_walk.html (self-contained) and e2e_walk.pdf (Chromium print).
    ...
    build_doc.py <run dir> <env.json>

env.json comes from env_facts.sh. An optional <run dir>/findings.json — a list of
{"title", "severity": "defect"|"note", "text"} written by the session for THAT run, with the
measurements in the text — becomes the Findings section; without it the section says so.
"""
...
rp = next((s for s in extra["steps"] if s["step"].startswith("Reports tab on ")), None)
lt = xby.get("light theme Overview")
...
if sw and sw.get("screenshot"):
    P(f"<p><span class='{'ok' if sw['ok'] else 'bad'}'>{'✓' if sw['ok'] else '✗'}</span> {e(sw['step'])} — {e(sw['detail'])}</p>")
    P(figure(RUN / "extra" / sw["screenshot"], f"{sw['step']} — {sw['detail']}", width=900))
if rp and rp.get("screenshot"):
    P(figure(RUN / "extra" / rp["screenshot"], f"{rp['step']} — {rp['detail']}", width=900))
if not sw:
    P("<p>Only one cluster is configured on this deployment; there is no second cluster to switch to.</p>")

P("<h2>9. Findings</h2>")
findings_path = RUN / "findings.json"
findings = json.loads(findings_path.read_text()) if findings_path.exists() else []
if not findings:
    P("<p>No product finding was recorded from this walk.</p>")
for i, f in enumerate(findings, 1):
    cls = "finding" if f.get("severity", "defect") == "defect" else "finding note"
    P(f"<div class='{cls}'><b>Finding {i} — {e(f['title'])}.</b> {e(f['text'])}</div>")
