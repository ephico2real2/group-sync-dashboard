...
def required_params(login_user: str, namespaces: str) -> dict[str, dict[str, str]]:
    """Values for the parameters that have no default. Everything else runs on its default, which
    is what an operator who clicks Generate without touching the form gets."""
    today = dt.date.today()
    return {
        "namespace-access": {"namespaces": namespaces},
        "access-certification": {"campaign": f"E2E walk {today.isoformat()}",
                                 "due": (today + dt.timedelta(days=30)).isoformat(),
                                 "reviewer": login_user},
    }
...
def walk_lookup(w: Walk) -> None:
    ...
    page.wait_for_selector("tr[data-ns], tr[data-group], #main .empty-note")
    page.wait_for_function(
        "() => [...document.querySelectorAll('.door .value')].every(v => v.textContent.trim() !== '…')")
...
def main() -> int:
    ...
    ap.add_argument("--namespaces", default="group-sync-dashboard,openshift-monitoring",
                    help="for the namespace-access report, which has no default (comma-separated)")
    args = ap.parse_args()
