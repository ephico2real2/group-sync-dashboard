...
2. **The cert-manager chain, before the workload.** `certmanager-tls.yaml` creates
   ...
   `ContainerCreating`.
3. **The workload.** `mock-openshift-deployment.yaml` and `mock-openshift-service.yaml`, both captured
   from the running cluster before it was deleted. The Service is ClusterIP on 6443.
4. **The credentials the dashboard reads**, as secret `mock-cluster-creds` with two keys:
...
5. **Wire it into the dashboard.** A volume `mock-creds` from that secret, mounted at `/etc/gsd/mock`.
